...
Для разворачивания приложения достаточно выполнить `make init`. Команда создаст контейнеры (с кодом и бд), применит миграции и загрузит фикстуры (продукты и купоны для примера работы). Чтобы запустить тесты можно воспользоваться командой `make test` (при запущеных контейнерах). В requests.http представлены примеры запросов.

# Решения
## Валидация
Валидация полей запроса реализована с помощью Symfony Validator. Для валидации налогового номера используется регулярка, дабы не усложнять логику валидации. Если будут регулярно добавляться новые страны, то можно будет создать отдельный валидатор для налогового номера.

## Использование платежных процессоров
Проведение платежа реализовано в PaymentService. Не стал создавать для них классы-прокладки, чтобы избежать избыточности кода.
Если понадобится добавлять новые платежные процессоры со сложной логикой, то нужно будет для них создать интерфейс PaymentProcessorInterface, вынести логику работы с ними в отдельные классы и добавить их в конфигурацию.
